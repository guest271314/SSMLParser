    ...
    class SpeechSynthesisSSMLParser {
      constructor(ssml) {
        ...
        this.dates = new Map(Object.entries({
          "m": "month",
          "d": "day",
          "y": "year"
        }));
        ...
        if (interpretAs === "date") {
          ...
          if (node.getAttribute("format")) {
            ...
            let {
              ...
              [this.dates.get(key)]: node.textContent.match(this.matchSayAsDateFormat)[index], ...o
            }), {});
            ...
            const text = `${format.includes("m") ? date.get("month") : ""} ` + `${format.includes("d") ? this.toOrdinal(date.get("day")).concat(format.includes("y") ? date.get("literal") : "")  : ""}` + `${format.includes("y") ? date.get("year") : ""}`;
